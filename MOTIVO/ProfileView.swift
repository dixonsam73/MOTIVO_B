...
fileprivate enum DiscoveryMode: Int, CaseIterable, Identifiable {
 ...
 struct ProfileView: View {
    ...
    // TEMPORARY — C-44 empirical gate (a). Remove with C44Probe.swift.
...
// TEMPORARY INSTRUMENTATION — C-44 empirical gate (a). Remove together with
         ...
         Section(header: Text("C-44 probe (temporary)").sectionHeader()) {
             VStack(alignment: .leading, spacing: 8) {
                 Button {
                     ...
                     navigationRow(title: "Run repeat-authorization probe")
                 }
                 .buttonStyle(.plain)
                 .contentShape(Rectangle())
                 .frame(minHeight: 44, alignment: .center)
