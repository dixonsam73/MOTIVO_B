begin;
-- A1 — B-39 PRODUCTION APPLY (20260914120000), guarded.
-- Generated 2026-09-15 by claude-evidence/scope012/gen_artifacts.py. DO NOT HAND-EDIT: regenerate.
--
-- Body: supabase/migrations/20260914120000_b39_revoked_status_ends_entitlement.sql, verbatim.
...
begin
  ...
  if n <> 6 then raise exception '[A1 B-39 apply PRE] expected exactly one function for each of the 6 targets, found %', n; end if;
  ...
  if v is distinct from '4c8733fbece816ed381f955a22b0f6f9' then raise exception '[A1 B-39 apply PRE] % md5 is %, expected %', 'connected_member', v, '4c8733fbece816ed381f955a22b0f6f9'; end if;
  ...
  if v is distinct from '77c80981c1fdbf2862c34b3e151e7f84' then raise exception '[A1 B-39 apply PRE] % md5 is %, expected %', 'membership_entitled_until', v, '77c80981c1fdbf2862c34b3e151e7f84'; end if;
  ...
  if v is distinct from 'd1b4d46af44f41953a97afae88e3f1f0' then raise exception '[A1 B-39 apply PRE] % md5 is %, expected %', 'membership_apply_state_v1', v, 'd1b4d46af44f41953a97afae88e3f1f0'; end if;
  ...
  if v is distinct from 'f730fb8adba2011f0600f16ef6883d5d' then raise exception '[A1 B-39 apply PRE] % md5 is %, expected %', 'account_privacy_requests_open', v, 'f730fb8adba2011f0600f16ef6883d5d'; end if;
  ...
  if v is distinct from '1f87a5832b702c369f169217b2eaceae' then raise exception '[A1 B-39 apply PRE] % md5 is %, expected %', 'account_privacy_self_v1', v, '1f87a5832b702c369f169217b2eaceae'; end if;
  ...
  if v is distinct from '13df8a41f6011aba4e68fdd5ea615065' then raise exception '[A1 B-39 apply PRE] % md5 is %, expected %', 'account_privacy_upsert_v1', v, '13df8a41f6011aba4e68fdd5ea615065'; end if;
  v := md5(coalesce(obj_description('public.membership_state(uuid)'::regprocedure, 'pg_proc'), ''));
  if v is distinct from '9ae51ea9798396177f2799754692c9ff' then raise exception '[A1 B-39 apply PRE] membership_state comment md5 is %, expected %', v, '9ae51ea9798396177f2799754692c9ff'; end if;
  ...
  if n <> 0 then raise exception '[A1 B-39 apply PRE] cached entitlement drift is % -- refusing; drift is assessed, never repaired here', n; end if;
...
-- ======================================================================== BODY
-- ===== BEGIN VERBATIM 20260914120000_b39_revoked_status_ends_entitlement.sql =====
...
begin
  v := (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname = 'connected_member');
  if v is distinct from '8e39f78402ec6402f690bcfa8935a8d4' then raise exception '[A1 B-39 apply POST] % md5 is %, expected %', 'connected_member', v, '8e39f78402ec6402f690bcfa8935a8d4'; end if;
  v := (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname = 'membership_apply_state_v1');
  if v is distinct from 'e77f869be62c319012203cb2fafbbe2d' then raise exception '[A1 B-39 apply POST] % md5 is %, expected %', 'membership_apply_state_v1', v, 'e77f869be62c319012203cb2fafbbe2d'; end if;
  v := (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname = 'membership_entitled_until');
  if v is distinct from 'e74146e4d9e5dc752e7fec34a3d3797f' then raise exception '[A1 B-39 apply POST] % md5 is %, expected %', 'membership_entitled_until', v, 'e74146e4d9e5dc752e7fec34a3d3797f'; end if;
  v := (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname = 'account_privacy_requests_open');
  if v is distinct from 'f730fb8adba2011f0600f16ef6883d5d' then raise exception '[A1 B-39 apply POST] % md5 is %, expected %', 'account_privacy_requests_open', v, 'f730fb8adba2011f0600f16ef6883d5d'; end if;
  v := (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname = 'account_privacy_self_v1');
  if v is distinct from '1f87a5832b702c369f169217b2eaceae' then raise exception '[A1 B-39 apply POST] % md5 is %, expected %', 'account_privacy_self_v1', v, '1f87a5832b702c369f169217b2eaceae'; end if;
  v := (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname = 'account_privacy_upsert_v1');
  if v is distinct from '13df8a41f6011aba4e68fdd5ea615065' then raise exception '[A1 B-39 apply POST] % md5 is %, expected %', 'account_privacy_upsert_v1', v, '13df8a41f6011aba4e68fdd5ea615065'; end if;
  v := md5(coalesce(obj_description('public.membership_state(uuid)'::regprocedure, 'pg_proc'), ''));
  if v is distinct from '9ae51ea9798396177f2799754692c9ff' then raise exception '[A1 B-39 apply POST] membership_state comment md5 is %, expected %', v, '9ae51ea9798396177f2799754692c9ff'; end if;
  if (select md5(string_agg(p.proname || '(' || pg_get_function_identity_arguments(p.oid) || ')' || pg_get_functiondef(p.oid) || coalesce(array_to_string(p.proacl, ','), ''), '|' order by p.proname, pg_get_function_identity_arguments(p.oid))) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname <> all (array['connected_member', 'membership_entitled_until', 'membership_apply_state_v1'])) is distinct from current_setting('s12.others') then raise exception '[A1 B-39 apply POST] a non-target public function changed'; end if;
  if (select md5(coalesce(string_agg(schemaname || '.' || tablename || '.' || policyname || '|' || permissive || '|' || roles::text || '|' || cmd || '|' || coalesce(qual, '') || '|' || coalesce(with_check, ''), '#' order by schemaname, tablename, policyname), '')) from pg_policies where schemaname in ('public', 'storage')) is distinct from current_setting('s12.policies') then raise exception '[A1 B-39 apply POST] a policy changed'; end if;
  if (select md5(string_agg(p.proname || '=' || coalesce(array_to_string(p.proacl, ','), '<null>'), '|' order by p.proname)) from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace where ns.nspname = 'public' and p.proname in ('connected_member', 'membership_entitled_until', 'membership_apply_state_v1')) is distinct from current_setting('s12.acl') then raise exception '[A1 B-39 apply POST] EXECUTE grants on a target changed'; end if;
  if (select count(*)::text || ':' || md5(coalesce(string_agg(m.user_id::text || '|' || m.environment || '|' || coalesce(m.apple_status::text, '-') || '|' || coalesce(m.renewal_date::text, '-') || '|' || coalesce(m.pending_cleanup_at::text, '-') || '|' || coalesce(m.entitlement_ended_at::text, '-'), '#' order by m.user_id, m.environment, m.original_transaction_id), '')) from public.membership m) is distinct from current_setting('s12.membership') then raise exception '[A1 B-39 apply POST] membership rows, schedules or end dates changed'; end if;
  if (select coalesce(bool_or(enforcement_enabled)::text, 'null') from public.membership_control) is distinct from current_setting('s12.enforcement') then raise exception '[A1 B-39 apply POST] enforcement_enabled changed'; end if;
  if (has_function_privilege('anon', 'public.connected_member(uuid)', 'execute') or has_function_privilege('authenticated', 'public.connected_member(uuid)', 'execute') or has_function_privilege('anon', 'public.membership_entitled_until(uuid)', 'execute') or has_function_privilege('authenticated', 'public.membership_entitled_until(uuid)', 'execute')) then raise exception '[A1 B-39 apply POST] a client role can execute an entitlement function'; end if;
  n := ((select count(*) from public.posts p where p.owner_entitled_until is distinct from public.membership_entitled_until(p.owner_user_id)) + (select count(*) from public.post_shares s where s.owner_entitled_until is distinct from public.membership_entitled_until(s.owner_user_id)) + (select count(*) from public.follows f where f.followed_entitled_until is distinct from public.membership_entitled_until(f.followed_user_id)) + (select count(*) from public.account_directory d where d.entitled_until is distinct from public.membership_entitled_until(d.user_id)));
  if n <> 0 then raise exception '[A1 B-39 apply POST] cached entitlement drift is % after recompute', n; end if;
end
$s12post$;
