-- U6b-1 PRODUCTION APPLY — SELF-VERIFYING. 2026-09-01.
--
-- THIS DENIES NOTHING. `enforcement_enabled` ships FALSE and the guard below
-- ASSERTS it is false after the migration runs. Binding is a separate, later,
-- one-row flip (P6). P2 and P6 must never be combined: everything between them
-- is the evidence that P6 is safe.
--
...
--
-- The body is BYTE-IDENTICAL to supabase/migrations/20260901120000_u6b_binding.sql.
-- Nothing was edited to make a statement succeed; the only additions are the
-- transaction wrapper, the guard and the final SELECT.
--
-- PREREQUISITE: run 2026-09-01-u6b-rollback-noop-verify.sql first and see its
-- NOTICE. Do not proceed on an EXCEPTION.

begin;

-- Captured BEFORE the migration so the guard can assert what this file actually
-- claims -- that it does not touch the membership tables -- rather than pinning
-- production's particular row counts, which would make the file unrehearsable
-- anywhere else. The claim is "unchanged", not "equal to 1/1/16".
create temp table _u6b_before on commit drop as
...
-- ==================== BEGIN VERBATIM 20260901120000_u6b_binding.sql ====================
...
-- ===================== END VERBATIM 20260901120000_u6b_binding.sql =====================

-- ============================================================ THE GUARD
do $u6b_guard$
declare n int; m_before int; b_before int; c_before int;
begin
  select count(*) into n from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace where ns.nspname='public';
  if n <> 28 then raise exception 'ABORT: public holds % functions, expected 28', n; end if;

  select count(*) into n from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
   where ns.nspname='public' and p.proname='shadow_observe';
  if n <> 0 then raise exception 'ABORT: shadow_observe still exists'; end if;

  select count(*) into n from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
   where ns.nspname='public' and p.proname in ('enforcement_gate','enforcement_active',
        'membership_entitled_until','tg_set_entitled_until','tg_membership_propagate_entitled_until');
  if n <> 5 then raise exception 'ABORT: % of the 5 new functions exist', n; end if;

  select count(*) into n from pg_policies where schemaname in ('public','storage');
  if n <> 33 then raise exception 'ABORT: % policies, expected 33', n; end if;
  ...
  -- G10: the retained-attribution RPC must gate on NO subject predicate.
  select count(*) into n from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
   where ns.nspname='public' and p.proname='get_account_directory_by_user_ids'
     and (regexp_replace(p.prosrc,'--[^\n]*','','g') like '%connected_member%'
       or regexp_replace(p.prosrc,'--[^\n]*','','g') like '%entitled_until%');
  if n <> 0 then raise exception 'ABORT: get_account_directory_by_user_ids gained a subject predicate -- G10'; end if;

  -- No uuid-taking entitlement predicate is reachable by any client role.
  select count(*) into n from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
  ...
  select count(*) into n from information_schema.columns where table_schema='public'
    and ((table_name='posts' and column_name='owner_entitled_until')
      or (table_name='post_shares' and column_name='owner_entitled_until')
      or (table_name='follows' and column_name='followed_entitled_until')
      or (table_name='account_directory' and column_name='entitled_until'));
  if n <> 4 then raise exception 'ABORT: % of the 4 visibility columns exist', n; end if;

  select count(*) into n from pg_trigger t join pg_class c on c.oid=t.tgrelid
    join pg_namespace ns on ns.oid=c.relnamespace where ns.nspname in ('public','storage') and not t.tgisinternal;
  if n <> 10 then raise exception 'ABORT: % triggers, expected 10', n; end if;

  select count(*) into n from information_schema.key_column_usage
   where constraint_name='shadow_enforcement_stat_pkey' and column_name='enforced';
  if n <> 1 then raise exception 'ABORT: `enforced` is not in the telemetry primary key'; end if;

  -- DRIFT: stored must equal recomputed on all four tables.
  ...
  -- THE ONE THAT MATTERS MOST: this migration must not have bound anything.
  select count(*) into n from public.membership_control where id and enforcement_enabled = false;
  if n <> 1 then raise exception 'ABORT: enforcement_enabled is NOT false -- P2 must deny nothing'; end if;
  select count(*) into n from public.membership_control where u6b_bound_at is null;
  if n <> 1 then raise exception 'ABORT: u6b_bound_at is set -- P2 must not bind'; end if;

  -- The membership tables are not this migration's business: UNCHANGED, whatever
  -- they held when it started.
  select m, b, c into m_before, b_before, c_before from _u6b_before;
  if m_before <> (select count(*) from public.membership)
     or b_before <> (select count(*) from public.membership_binding)
     or c_before <> (select count(*) from public.membership_cutover) then
    raise exception 'ABORT: a membership table moved during the migration (was %/%/%)', m_before, b_before, c_before;
  end if;
end
$u6b_guard$;
...
-- Returns a ROW. "No rows returned" means the submission was not this file.
select
  ...
  (select count(*) from pg_policies where schemaname in ('public','storage')
     and (coalesce(qual,'')||coalesce(with_check,'')) like '%enforcement_gate%')                       as gated_policies,
  (select count(*) from pg_policies where schemaname in ('public','storage')
     and (coalesce(qual,'')||coalesce(with_check,'')) like '%enforcement_active%')                     as subject_gated_policies,
  (select count(*) from pg_trigger t join pg_class c on c.oid=t.tgrelid
     join pg_namespace n on n.oid=c.relnamespace where n.nspname in ('public','storage') and not t.tgisinternal) as triggers,
  (select enforcement_enabled from public.membership_control)                                          as enforcement_enabled,
  (select u6b_bound_at is null from public.membership_control)                                         as still_unbound,
  (select count(*) from public.posts where owner_entitled_until is not null)                           as posts_with_visibility,
  'U6b-1 APPLIED -- INERT. enforcement_enabled is FALSE; nothing is denied.'                           as status;
