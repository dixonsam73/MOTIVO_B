-- U6a ROLLBACK NO-OP VERIFICATION — RUN THIS AGAINST PRODUCTION IMMEDIATELY
-- BEFORE APPLYING THE U6a MIGRATION.
--
-- The body below is byte-identical to 2026-09-01-u6a-rollback-baseline-production.sql.
-- Both are emitted by one generator from one production capture, so they cannot
-- drift apart.
--
-- WHAT THIS PROVES, and it is three things rather than one:
--
--   1. The rollback file still matches production. The BEFORE fingerprints are
--      asserted against the values captured when the file was generated, so any
--      drift between capture and deploy STOPS the run instead of silently
--      shipping a rollback to a state that no longer exists.
--   2. Applying it is a no-op. The AFTER fingerprints must equal the BEFORE
--      fingerprints. A rollback that changes something when nothing needs
--      changing is not a rollback, it is an unreviewed migration.
--   3. THE SESSION CAN ACTUALLY ALTER ALL 33 POLICIES — including the ten on
--      storage.objects, which are owned by supabase_storage_admin. This is the
--      half a fingerprint comparison alone would not give: it exercises the
--      exact privilege the migration needs, before the migration needs it, on a
--      statement whose failure costs nothing.
--
-- ONE SUBMISSION, GUARD INSIDE, per the standing rule in supabase/README.md.
-- RAISE EXCEPTION aborts this transaction, so a mismatch rolls itself back and
-- reports the value it saw. No human decision sits between the observation and
-- the outcome.
--
-- EXPECTED OUTPUT: `NOTICE:  U6a rollback baseline verified as a byte-identical
-- no-op` and nothing else. Any EXCEPTION is a STOP — do not apply the migration.

begin;

create temp table _u6a_fp on commit drop as
select (select md5(string_agg(schemaname||'|'||tablename||'|'||policyname||'|'||cmd||'|'||coalesce(qual,'')||'|'||coalesce(with_check,''), E'\n' order by schemaname, tablename, policyname)) from pg_policies where schemaname in ('public','storage')) as pol,
...
begin
  select pol, fn into p, f from _u6a_fp;
  if p <> '7d6c09eb1921878fd98b0ebd9fe9f664' then
    raise exception 'ABORT: production policy fingerprint is %, expected 7d6c09eb1921878fd98b0ebd9fe9f664. Production has drifted since this file was generated — regenerate it, do not proceed.', p;
  end if;
  if f <> '48e8cc60b6ca66d102ce10cfec70a19c' then
    raise exception 'ABORT: production function fingerprint is %, expected 48e8cc60b6ca66d102ce10cfec70a19c. Production has drifted since this file was generated — regenerate it, do not proceed.', f;
  end if;
end
$u6a_pre$;
...
do $u6a_post$
declare p_before text; f_before text; p_after text; f_after text;
begin
  select pol, fn into p_before, f_before from _u6a_fp;
  select (select md5(string_agg(schemaname||'|'||tablename||'|'||policyname||'|'||cmd||'|'||coalesce(qual,'')||'|'||coalesce(with_check,''), E'\n' order by schemaname, tablename, policyname)) from pg_policies where schemaname in ('public','storage')) into p_after;
  select (select md5(string_agg(pg_get_functiondef(p.oid), E'\n' order by p.proname)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('add_post_comment','follow_requests_open','get_account_directory_by_user_ids','get_unread_private_comment_groups','has_unread_private_comments','mark_post_comments_viewed','reply_to_commenter','respond_to_commenters','search_account_directory'))  into f_after;
  if p_after <> p_before then
    raise exception 'ABORT: applying the rollback baseline CHANGED the policies (% -> %). It is not a no-op.', p_before, p_after;
  end if;
  if f_after <> f_before then
    raise exception 'ABORT: applying the rollback baseline CHANGED the functions (% -> %). It is not a no-op.', f_before, f_after;
  end if;
  raise notice 'U6a rollback baseline verified as a byte-identical no-op (policies %, functions %)', p_after, f_after;
end
$u6a_post$;
