-- U6a ROLLBACK BASELINE — the pre-U6a definitions, captured BEFORE any change.
--
-- U6a is NOT purely additive: it MODIFIES 23 existing policies and 9 existing
-- functions. So rollback cannot be "drop what was added", and the only honest
-- rollback is a restoration of what was there. This file IS that restoration.
--
-- GENERATED, NOT HAND-WRITTEN. Produced by reading pg_policies and
-- pg_get_functiondef on the local B-23 reproduction at a clean migrated state
-- (supabase db reset --local, all four migrations applied, 33 policies), on
-- 2026-08-30. Regenerating it on a clean reset must reproduce it byte for byte.
--
-- ALL 33 POLICIES ARE CAPTURED, not only the 23 U6a touches. Capturing exactly
-- the planned set would make the rollback correct only if the plan was correct,
-- which is the one thing a rollback must not assume.
--
