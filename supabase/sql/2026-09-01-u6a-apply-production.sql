-- U6a PRODUCTION APPLY — SELF-VERIFYING. 2026-09-01.
--
-- WHY THIS FILE EXISTS, and it is not a convenience.
--
-- The first attempt at P2 reported "Success. No rows returned." and applied
-- NOTHING. Production's policy and function fingerprints were unmoved and no new
-- object existed. Three probes then established that the SQL editor writes DDL
-- fine, runs multi-statement submissions fine, and honours BEGIN/COMMIT fine --
-- so the mechanism was never the problem and the submitted TEXT was.
--
-- A success message is not evidence that the intended text ran. This file makes
-- that gap unreachable: the guard below asserts the post-migration state INSIDE
-- the same transaction, so a submission that ran the wrong content ABORTS and
-- says what it saw, instead of committing nothing and reporting success. The
-- reasoning is the standing rule in supabase/README.md -- one submission, guard
-- inside, no human decision between the observation and the outcome.
--
-- The migration body below is BYTE-IDENTICAL to
-- supabase/migrations/20260830120000_u6a_shadow.sql. Nothing was edited to make
-- a statement succeed; the only additions are the transaction wrapper, the
-- guard, and the two probe drops.
--
-- EXPECTED RESULT: a ONE-ROW TABLE at the end, not "no rows returned".
-- Read those numbers back rather than the success message.

begin;

-- Housekeeping: remove the two diagnostic tables the probes created. Inside the
-- transaction deliberately, so an aborted migration leaves them alone too.
drop table if exists public._u6a_editor_probe;
drop table if exists public._u6a_editor_probe2;

-- ==================== BEGIN VERBATIM 20260830120000_u6a_shadow.sql ====================
...
-- ===================== END VERBATIM 20260830120000_u6a_shadow.sql =====================

-- ============================================================ THE GUARD
--
-- Every assertion is on state this transaction just produced. RAISE EXCEPTION
-- aborts the transaction, so a wrong number rolls itself back and reports what
-- it saw. There is no branch here that a human has to notice.
do $u6a_guard$
declare
  n_fns int; n_pols int; n_obs_pols int; n_obs_fns int;
  has_tbl int; has_obs int; has_self int;
  p_fp text; f_fp text;
begin
  ...
  if n_fns <> 24 then
    raise exception 'ABORT: public holds % functions, expected 24. The migration did not run, or did not run whole.', n_fns;
  end if;

  select count(*) into has_obs from pg_proc p join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='public' and p.proname='shadow_observe';
  select count(*) into has_self from pg_proc p join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='public' and p.proname='connected_member_self';
  select count(*) into has_tbl from information_schema.tables
   where table_schema='public' and table_name='shadow_enforcement_stat';
  if has_obs <> 1 or has_self <> 1 or has_tbl <> 1 then
    raise exception 'ABORT: new objects missing -- shadow_observe %, connected_member_self %, shadow_enforcement_stat %.', has_obs, has_self, has_tbl;
  end if;

  select count(*) into n_pols from pg_policies where schemaname in ('public','storage');
  if n_pols <> 33 then
    raise exception 'ABORT: % policies, expected 33. U6a adds and removes none.', n_pols;
  end if;

  select count(*) into n_obs_pols from pg_policies
   where schemaname in ('public','storage')
     and (coalesce(qual,'') like '%shadow_observe%' or coalesce(with_check,'') like '%shadow_observe%');
  if n_obs_pols <> 23 then
    raise exception 'ABORT: % policies carry the observer, expected 23.', n_obs_pols;
  end if;

  select count(*) into n_obs_fns from pg_proc p join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='public' and p.prosrc like '%shadow_observe%'
     and p.proname in ('add_post_comment','follow_requests_open','get_account_directory_by_user_ids',
                       'get_unread_private_comment_groups','has_unread_private_comments',
                       'mark_post_comments_viewed','reply_to_commenter','respond_to_commenters',
                       'search_account_directory');
  if n_obs_fns <> 9 then
    raise exception 'ABORT: % of the 9 RPCs carry the observer.', n_obs_fns;
  end if;

  -- THE STRONGEST ASSERTION IN THIS FILE. These are the fingerprints of the
  -- LOCAL instance after the identical migration, rehearsed on 2026-09-01. The
  -- pre-U6a fingerprints already agreed exactly across both instances, so a
  -- match here means production's 33 policies and 9 functions are byte-identical
  -- to the build the 74-assertion suite ran against -- not merely "23 policies
  -- mention the observer".
  select (select md5(string_agg(schemaname||'|'||tablename||'|'||policyname||'|'||cmd||'|'||coalesce(qual,'')||'|'||coalesce(with_check,''), E'\n' order by schemaname, tablename, policyname)) from pg_policies where schemaname in ('public','storage')) into p_fp;
  ...
  if p_fp <> 'bad893a9528b5b57bc5e73efd905a6ab' then
    raise exception 'ABORT: post-migration policy fingerprint is %, expected bad893a9528b5b57bc5e73efd905a6ab.', p_fp;
  end if;
  if f_fp <> '052ee5ced5b6ae0af2e70ed12e1bd359' then
    raise exception 'ABORT: post-migration function fingerprint is %, expected 052ee5ced5b6ae0af2e70ed12e1bd359.', f_fp;
  end if;
end
$u6a_guard$;
...
-- Returns a ROW. "No rows returned" here means the submission was not this file.
select
  ...
  (select count(*) from pg_policies where schemaname in ('public','storage')
     and (coalesce(qual,'') like '%shadow_observe%'
       or coalesce(with_check,'') like '%shadow_observe%'))              as observed_policies,
  (select count(*) from public.shadow_enforcement_stat)                  as shadow_rows,
  'U6a APPLIED AND GUARD PASSED'                                         as status;
