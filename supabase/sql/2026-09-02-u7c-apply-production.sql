-- U7d P1 — APPLY U7c TO PRODUCTION. ONE TRANSACTION.
--
...
-- is applied.
--
-- THIS FILE DELETES NOTHING and touches no user data. It adds three functions
-- and one grant, and replaces one selector so that eligibility has exactly ONE
-- definition shared by the preview and execute paths.
--
-- RUN THIS ONLY AFTER U7b's APPLY FILE HAS SUCCEEDED. It replaces
-- membership_due_for_cleanup_v1, which U7b creates.
...
begin
  select count(*) into n from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
   where ns.nspname='public' and p.proname in
     ('membership_cleanup_lease_v1','membership_cleanup_eligible_v1','membership_cleanup_authorised_v1');
  if n <> 3 then raise exception 'U7c guard: expected 3 new functions, found %', n; end if;

  -- STABLE is the structural guarantee that the PREVIEW PATH CANNOT CLAIM.
  if (select provolatile from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
       where ns.nspname='public' and p.proname='membership_cleanup_eligible_v1') <> 's'
  then raise exception 'U7c guard: the eligibility/preview function is not STABLE'; end if;

  if not has_function_privilege('service_role','public.membership_cleanup_eligible_v1(integer)','execute')
  or not has_function_privilege('service_role','public.membership_cleanup_authorised_v1(uuid)','execute')
  then raise exception 'U7c guard: service_role cannot execute the new functions'; end if;

  -- The lease is an implementation detail and is granted to NOBODY.
  if has_function_privilege('service_role','public.membership_cleanup_lease_v1()','execute')
  or has_function_privilege('authenticated','public.membership_cleanup_lease_v1()','execute')
  or has_function_privilege('anon','public.membership_cleanup_lease_v1()','execute')
  then raise exception 'U7c guard: the lease function was granted to a role'; end if;

  -- B-33 MUST SURVIVE THIS DEPLOY. The membership oracle stays unbuildable:
  -- no role, service_role included, may evaluate connected_member(uuid).
  if has_function_privilege('service_role','public.connected_member(uuid)','execute')
  or has_function_privilege('authenticated','public.connected_member(uuid)','execute')
  or has_function_privilege('anon','public.connected_member(uuid)','execute')
  then raise exception 'U7c guard: B-33 VIOLATED -- connected_member(uuid) is now reachable'; end if;

  select count(*) into n from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
   cross join (values ('anon'),('authenticated')) r(role)
   where ns.nspname='public' and p.proname like 'membership_cleanup%'
     and has_function_privilege(r.role, p.oid, 'execute');
  if n <> 0 then raise exception 'U7c guard: a client role can execute a cleanup function'; end if;

  -- ONE eligibility definition: the execute selector consults the preview one.
  if (select count(*) from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
       where ns.nspname='public' and p.proname='membership_due_for_cleanup_v1'
         and pg_get_functiondef(p.oid) ~ 'membership_cleanup_eligible_v1') <> 1
  then raise exception 'U7c guard: the execute selector does not consult the eligibility definition'; end if;

  if (select count(*) from public.membership) <> 1 then
    raise exception 'U7c guard: membership row count changed'; end if;
  if (select pending_cleanup_at from public.membership)
     <> timestamptz '2026-11-01 15:16:44+00' then
    raise exception 'U7c guard: pending_cleanup_at was altered by the deploy'; end if;
end $$;

-- ============ FINAL SELECT — this MUST return exactly one row, or nothing ran
select 'U7c APPLIED' as result,
       (select count(*) from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
         where ns.nspname='public' and p.proname like 'membership_cleanup%') as cleanup_functions,
       (select provolatile from pg_proc p join pg_namespace ns on ns.oid=p.pronamespace
         where ns.nspname='public' and p.proname='membership_cleanup_eligible_v1') as preview_volatility,
       (select has_function_privilege('service_role','public.connected_member(uuid)','execute')) as b33_oracle_reachable,
