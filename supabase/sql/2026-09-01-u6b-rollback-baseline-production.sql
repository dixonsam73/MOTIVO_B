-- U6b ROLLBACK BASELINE — GENERATED FROM **PRODUCTION**, 2026-09-01, BEFORE U6b-1.
--
-- 33 policies and 10 functions. The tenth is `shadow_observe`, which U6b DROPS:
-- a rollback that restored the 23 policies but not the function they call would
-- leave every one of them referencing something that does not exist. Capturing
-- ALL 33 policies rather than the 23 U6b touches is the same rule as before --
-- capturing exactly the planned set would make the rollback correct only if the
-- plan was correct.
--
-- Captured from production's own catalog, pre-U6b:
--   policies   33   fingerprint bad893a9528b5b57bc5e73efd905a6ab
--   functions  10   fingerprint bc08dc4fcd953ce9d063b3801d65dccf
--
-- This file does NOT drop U6b's additive objects. Those DROPs are in the
-- deployment README's rollback section, and they must run AFTER this file.
--
-- FUNCTIONS ARE RESTORED BEFORE POLICIES, AND A LOCAL REHEARSAL IS WHY.
-- The first generated ordering put policies first and failed with
-- `function shadow_observe(text) does not exist` -- U6b DROPS that function and
-- 23 restored policies reference it. shadow_observe is emitted before the nine
-- RPCs for the same class of reason: check_function_bodies validates
-- SQL-language bodies at CREATE time, so follow_requests_open cannot be created
-- before the function it calls.
--
-- Verify it as a no-op first: 2026-09-01-u6b-rollback-noop-verify.sql.
