...
is A67c "$(psq "select count(*) from pg_policies where schemaname in ('public','storage') and (coalesce(qual,'')||coalesce(with_check,'')) ~* 'connected_member|membership|enforcement_gate';")" "23" "the 23 enumerated policies consult shadow telemetry"
...
is A67c3 "$(psq "select public.enforcement_gate('probe.a67c')::text;")" "true" "...and the observer is inert: it returns true, always"
