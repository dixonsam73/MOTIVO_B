...
# AMENDED AGAIN FOR U7c, 2026-09-02: membership_cleanup_eligible_v1 and
# membership_cleanup_authorised_v1 are now reachable by service_role.
# membership_cleanup_lease_v1 is deliberately ABSENT — it is granted to nobody,
# and its absence here is the evidence of that.
#
# IT MOVED TWICE IN TWO UNITS, AND THE SECOND MOVE WAS NOT PREDICTED even though
# the first had just demonstrated the rule. That is the finding, not the edit:
# this line moves on EVERY new service_role grant, so any unit adding one should
...
is A45  "$(psq "select string_agg(p.proname,',' order by p.proname) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname like 'membership%_v1' and has_function_privilege('service_role',p.oid,'EXECUTE');")" "membership_apply_reconciliation_v1,membership_cleanup_authorised_v1,membership_cleanup_complete_v1,membership_cleanup_eligible_v1,membership_due_for_cleanup_v1,membership_due_for_reconciliation_v1,membership_establish_v1,membership_ingest_notification_v1,membership_record_reject_v1" "service_role EXECUTE set"
...
#
# REWRITTEN AT U6a, 2026-08-30. THE OLD FORM WAS ACCIDENTALLY GREEN.
#
# It read: count of policies whose qual matches 'connected_member|membership',
...
# the enumerated policies consult SHADOW TELEMETRY, and enforcement remains
