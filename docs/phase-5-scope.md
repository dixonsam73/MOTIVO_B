...
| ~~**P5-N Tier 1**~~ | **Accessibility & polish** | **TIER 1 COMPLETE 2026-09-09 — IMPLEMENTATION ONLY; DEVICE VOICEOVER VERIFICATION OUTSTANDING.** C-11 and C-67 both resolved; `docs/phase-5-n-tier1-acceptance.md` §5 is the device checklist. **TIER 2 IS AN UNVERIFIED CANDIDATE INVENTORY, NOT A DEFECT COUNT AND NOT A QUALITY METRIC** — ~49 candidate unlabelled icon-only controls across 18 other files, calibrated in `docs/phase-5-n-scope-proposal.md` §3, to be triaged only in a later dedicated pass if wanted. **The heuristic cannot detect MISLABELS — C-11's own class — so that population is unmeasured.** Dynamic Type, contrast, focus order, rotor and any conformance claim remain explicitly OUT. Original scope: **C-11** VoiceOver mislabel · **C-67** the attachment viewer's transport controls carry **no** accessibility labels at all (filed 2026-09-09 by P5-M's scoping; **not** C-11, which is a mislabel in a different file) · remaining polish | none |
