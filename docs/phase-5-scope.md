...
> **RE-SCOPE 2026-09-18. READ FIRST.** Connected is to become an **18+ service
> only**; Solo is unchanged and carries no Études-imposed age restriction. The
> 13–17 Connected target of the CP workstream is **superseded as direction**.
> CP-0…CP-3 (P5-C, P5-B/P5-D, P5-E, P5-F) **stay closed as historical work**, and
> their records are not rewritten. **P5-G and P5-H are re-scoped** in the table
> below. The shipped code and production still implement the 13+ design until a
> reviewed replacement ships. **No implementation is authorised.** Decision record:
> `docs/adult-only-connected-rescope-2026-09-18.md`. **The rest of Phase 5 is
> unaffected.**
...
| **P5-G** | 👤🚪 **CP-4** | **RE-SCOPED 2026-09-18 — NOW THE LEGAL AND EVIDENTIAL DETERMINATION FOR ADULT-ONLY CONNECTED. NOT STARTED IN THIS FORM; NO IMPLEMENTATION AUTHORISED.** It must determine: which Apple age-assurance result, if any, SD Songs Ltd finds adequate for Ofcom HEAA (**generic `.confirmed` is not assumed adequate**); what the server may trust, given that the Apple result is delivered to the app; the iOS 26.4/26.5 consequence; existing identities and re-assurance; payment relative to eligibility; and any remaining regional or `RESCIND_CONSENT` obligation. See `docs/adult-only-connected-rescope-2026-09-18.md` §5; the principles carried forward are in the decision register's 2026-09-18 carry-forward section. **The 13+ record that follows is historical and is not rewritten:** **DECISIONS COMPLETE 2026-09-09; DPIA AND LEGAL OUTSTANDING.** Six account-holder decisions recorded in `docs/phase-5-g-decision-register.md` §A. **Q1 IS MINIMAL: the established age band is RETAINED and re-derived ONLY on deliberate member action. Periodic re-derivation (CONTINUOUS) was explicitly REJECTED and its implementation REVERTED** (`cbaeeed`; preserved at `c5440d8`) — no automatic refresh, no cadence, no throttle state, no withholding state and no new server state. Grounds, established from Apple's SDK and documentation rather than assumed: **age moves one way**, so the only ordinary reclassification is 13-17 → 18+, whose late observation leaves a member MORE protected; **Apple requires no re-check** after establishment and refers compliance to counsel; and *"the API will be called often"* does **not** entail that Études must poll. **Q2** teens cannot enable inbound follow requests (deliberate policy, no control to be added); **Q3** `communicationLimits` not consulted; **Q4** declined/unavailable stays fail-closed and retryable; **Q5** minimisation kept, no DOB and no provenance; **Q6 A only** — the legacy directory row is repaired through the ordinary path and `tg_directory_requires_band` is **NOT** widened to UPDATE, on a reachability analysis showing no supported post-CP lifecycle can produce the state. **Remaining: C1–C4 to LEGAL** (refusal wording, DPIA residual assurance, the teen limitation verbatim, and scope/ongoing-duty/`RESCIND_CONSENT` applicability). **Release-gating but NOT blocking P5-G:** Q6/A execution, and the Production ASSN Server URL together with `APPLE_ASSN_ALLOWED_ENVIRONMENTS` (§N) | designs P5-B…P5-F settled; **runs in parallel** |
| **P5-H** | 👤🚪 **CP-5** | **2026-09-18: WAITS until the adult-only age architecture and P5-G's legal position are settled.** Its content changes accordingly. `[AGE]`, the policy and the ASC label re-derivation must describe Solo without an Études-imposed age restriction and Connected as 18+ only. The app's App Store age rating must be **re-assessed** for that split; that is an open question (decision record §5.7), not a decision that the app is rated 18+. Original scope, unchanged: resolve `[AGE]` → publish `etudes.app/privacy` → **then** publish the ASC labels | P5-G |
| ~~**P5-I**~~ | **C-34** | **IMPLEMENTATION COMPLETE 2026-09-11 — DEVICE VERIFICATION CARRIED; C-34 NOT CLOSED.** The image-cache TTL was **superseded** by Phase 4's version signal and not built. What shipped: directory rows expire after **20 min** (consistent with `CommentPresenceStore`) so the version reaches every site; and the owner's local avatar copy follows the backend when no unsynced local change exists. **B-38** filed. The device check needs the Production Connected fixture. `docs/phase-5-i-acceptance.md`. *Original scope: avatar cache TTL; completes the work whose version-signal half shipped in Phase 4* | none |
| ~~**P5-J**~~ | **Correctness / safety** | **COMPLETE 2026-09-10 as P5-J′ — no device acceptance required.** **C-6** closed: accepted fail-closed, migration compatibility release-time verified across all 10 model versions (empty-store scope) · **C-16** fixed · **C-20** fixed — its 2026-09-09 demotion was wrong · **C-21** closed as refuted. **C-81** filed. New warning baseline **Debug 177 / Release 165**; 215/215 structured. `docs/phase-5-j-prime-acceptance.md`. *Original scope: C-6 `fatalError` on store load · C-16 `try!` on directory creation · C-20 main-actor isolation · C-21 discarded status reads* | none |
...
| ~~**P5-M**~~ | **Product** | **PLAYBACK-SPEED CONTROL — COMPLETE AND DEVICE-VERIFIED 2026-09-09. 14/14 device checks green, including PITCH HEARD at 0.5× and 2×.** The device pass found and fixed two defects: a **missed video play path** (`requestPlay()`, reached from the big centre play button, used bare `play()` and so always ran at 1×) and **rate selection failing during playback** in both media, traced to the transport row being rebuilt up to 30×/s. A third reported failure — paging persistence — **was not a defect**: my check wording was ambiguous and had been read as closing the viewer. **Learned from the device:** audio and video use SEPARATE viewer instances, so "per viewer session" is per media type in practice. `docs/phase-5-m-playback-speed-acceptance.md`, `docs/phase-5-m-device-defects-prediction.md`. **C-69** filed for the unreachable remote-audio path | none |
| ~~**P5-N Tier 1**~~ | **Accessibility & polish** | **TIER 1 COMPLETE AND DEVICE/VOICEOVER-VERIFIED 2026-09-09 — all five device checks GREEN.** C-11 and C-67 both resolved; `docs/phase-5-n-tier1-acceptance.md` §5 is the device checklist. **TIER 2 IS AN UNVERIFIED CANDIDATE INVENTORY, NOT A DEFECT COUNT AND NOT A QUALITY METRIC** — ~49 candidate unlabelled icon-only controls across 18 other files, calibrated in `docs/phase-5-n-scope-proposal.md` §3, to be triaged only in a later dedicated pass if wanted. **The heuristic cannot detect MISLABELS — C-11's own class — so that population is unmeasured.** Dynamic Type, contrast, focus order, rotor and any conformance claim remain explicitly OUT. Original scope: **C-11** VoiceOver mislabel · **C-67** the attachment viewer's transport controls carry **no** accessibility labels at all (filed 2026-09-09 by P5-M's scoping; **not** C-11, which is a mislabel in a different file) · remaining polish | none |
...
**SUPERSEDED IN PART 2026-09-18.** The requirement to describe **distinct 13–17
privacy defaults** no longer applies to the target product, because Connected is
to become 18+ only. The policy must describe whatever age-assurance data the
adult-only architecture actually processes, which is not yet decided. The
re-derivation rule above still stands. See
