...
| ~~**P5-M**~~ | **Product** | **PLAYBACK-SPEED CONTROL — IMPLEMENTATION COMPLETE; TWO DEVICE DEFECTS FOUND AND FIXED 2026-09-09, BOTH CONFIRMED GREEN ON DEVICE; REMAINING ACCEPTANCE SEQUENCE OUTSTANDING.** The device pass found (i) a **missed video play path** — `requestPlay()`, reached from the big centre play button, still used bare `play()` and so always ran at 1× — and (ii) **rate selection failing during playback** in both media, traced to the transport row being rebuilt up to 30×/s by 0.1s time observers and a 30fps waveform timer. Fixed and re-verified. **Pitch, pause/resume retention, paging persistence, viewer reset and VoiceOver are NOT yet dispositioned** — five rates (0.5/0.75/1/1.5/2), 1× default, ephemeral per viewer session, compact `Menu` in both existing transport rows, pitch preserved on all three player stacks. `docs/phase-5-m-playback-speed-acceptance.md`. Original scope: **playback-speed control** (AttachmentViewerView only, local and remote audio/video, discrete 50/75/100%, pitch preserved, no looping, no `PracticeTimerView` changes, no `MediaTrimView` carry-over, TestFlight soak). **C-3 IS NO LONGER LISTED HERE — corrected 2026-09-09.** Pairing it with playback speed implied a dependency that does not exist: **C-3 is `PracticeTimerView` foreground hydration, playback speed is `AttachmentViewerView`, they touch disjoint files, and this row already FORBIDS `PracticeTimerView` changes during the playback-speed work.** So the two could never have been done together under this row's own terms. **Playback speed proceeds independently of C-3** | none |
| **P5-N** | **Accessibility & polish** | **C-11** VoiceOver mislabel · **C-67** the attachment viewer's transport controls carry **no** accessibility labels at all (filed 2026-09-09 by P5-M's scoping; **not** C-11, which is a mislabel in a different file) · remaining polish | none |
