...
| **U6b** bind | Is it safe to make enforcement binding? **RESTATED 2026-09-01.** Requires **G11 passed** — **DONE, 16 of 16 on 2026-09-01** — **and** the grandfather-dependent population **discharged by CENSUS**. The old wording, *zero decisions resting solely on the grandfather clause*, is a criterion satisfied by OBSERVING NOTHING, and **B-34 established three ways to observe nothing while the traffic is real**. The shadow window is now ONE-DIRECTIONAL: non-zero BLOCKS and is trustworthy; zero can never UNBLOCK. **Census, 2026-09-01: 1 of 16 pre-cutover identities holds authoritative membership state, 15 do not — so U6b is BLOCKED on measured evidence.** Snapshot stays active as backstop | That the snapshot is removable |
| **U6c** remove | Is the migration provenance safe to delete? **Either** every snapshot UID has authoritative membership state **or** twelve months have elapsed since U6b — **and in either case** a final LIVENESS check. **Any snapshot UID ACTIVE since U6b but still lacking authoritative membership state is evidence that U5 failed for that identity and blocks removal until dispositioned.** **CORRECTED 2026-09-01 by B-35: this said `auth.users.last_sign_in_at`, which GoTrue advances only on re-authentication and never on token refresh — an identity on a long-lived session reads as dormant while it is actively issuing requests. Key on `max(auth.sessions.updated_at)`, cross-checked against `auth.users.updated_at`. It fails OPEN, so a blind check removes the snapshot out from under identities still relying on it.** Post-phase, dated, owned on B-11 | — |
