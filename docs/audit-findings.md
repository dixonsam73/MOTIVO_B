...
Phase 1 rather than by the audit. No finding from the audit itself is Resolved
yet.
...
| C-1 | Entitlement resolution cannot express "indeterminate"; one negative read triggers irreversible backend deletion. `ConnectedMembershipStore:230-267` → `MOTIVOApp:339/377` | P0 | **Confirmed defect.** Device evidence (Release, instrumented, 2026-08-09), reproduced in two independent runs: a forced refresh fired by `transactionUpdate.verified` immediately after a *verified purchase* published `notEntitled`; only the second refresh moments later published `entitled`. A negative read can therefore be spurious rather than authoritative — previously argued from structure, now observed. Corollary: `purchase()`'s `guard isEntitled` (`:133`) is reliable only because a second read follows. **Environment:** both observations were under the synthetic StoreKit configuration, not real StoreKit (C-9), so the *frequency* under real conditions is still unknown — the mechanism is established, the rate is not. **Client half landed** on `feature/solo-connected` (`fix: remove client authority for membership-expiry deletion`): the detect → arm → gate → delete pipeline is gone, `handleMembershipState` now does nothing but `applyActivation`, and no client entitlement path can reach `deleteCurrentConnectedAccount`. No indeterminate state was added, and none is needed: without deletion a spurious negative read only withdraws access, which invariant 3 permits on client evidence because it is reversible. Not Resolved — the server half (App Store Server Notifications) is Phase 3, and device verification of a real lapse is outstanding. **Interim consequence: no membership-expiry cleanup happens at all until Phase 3 lands; lapsed members' backend data is retained.** Accepted deliberately — retention pending authoritative evidence beats deletion on evidence twice observed to be wrong | 1 (client) / 3 (server) |
