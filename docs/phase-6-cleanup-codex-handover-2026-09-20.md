...
>
> Instructions to a fresh Codex window for the same bounded cleanup, prepared after `dec0236`.
>
> The cleanup was completed and committed as `e5b3b08`. **Read this as a record of how the window was set up, not as instructions to follow.**
>
...
# Études — fresh Codex window: bounded cleanup, then Phase 6 disposition

Prepared by Codex on 20 September 2026, after Samuel reported dec0236 pushed.

You are the independent reviewer/coordinator. Samuel asked for fresh windows for both
agents after C-70. Start by verifying checkout/protected state and locating the NEW real
Claude task after Samuel opens it. Ask Claude for the reachability scope/evidence, not
implementation yet. Independently trace the candidate graph while Claude scopes. Once
proof is adequate, you may authorise bounded removal under Samuel's standing delegation.
Review actual diff and proportionate tests; hold for Samuel's commit/push instruction.
Do not create a new user task automatically; this document is for a manually opened
fresh window. No additional work in the old window is in flight.
