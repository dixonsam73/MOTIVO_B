...
| **CP-2** | **Server** — discovery clause on `search_account_directory`, **plus REQUIREMENT CP-2-R1** | CP-1 | **Must not touch `get_account_directory_by_user_ids`** — G10 / U7. Its acceptance suite re-asserts the U7 invariant. **CP-2-R1 IS BINDING, NOT INCIDENTAL:** `follow_requests_open` ends `coalesce(..., true)`, so a **missing or unresolved row resolves PERMISSIVE** — a contact vector into a possible minor, open by absence. It **must** resolve closed, and it belongs in CP-2's predicted surface and acceptance criteria. The behaviour change (identities with no directory row move to requests-closed) must be **counted before applying** |
